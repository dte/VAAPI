...
int decode(void)
{
    ...
    // PS:Comment what is an iq matrix?
    H264IQMatrix h264_iq_matrix;
    // PS:Comment does slice data include the slice header?
    // how do i distinguish between the slice header and the slice data?
    // are multiple slices supported?
    ...
    // PS:Comment this data is going to be replaced by the annex-b style h264 stream from paperspace
    // what happens to sps and pps? are they part of slice data?
    ...
    // PS:Comment most of this copy calls dont make sense to me. i need to know how to extract the information from the video stream
#define COPY(field) \
...
#define COPY_BFM(a,b,c) \
    ...
    // PS:Comment do i have to keep track of reference pictures?
    for (i = 0; i < 16; i++)
