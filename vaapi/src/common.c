...
int doTheThing()
{
    ...
    // PS:Comment is there any performance inpact using rgb color space?
    ...
    // PS:Comment the display part makes sure the image is visible on the screen
    // is this implementation the fastest way to do this?
    if (display() < 0) {
        fprintf(stderr, "ERROR: display failed\n");
        goto end;
    }

    // PS:Comment i assume post is to be called only once before ending the program
    if (post() < 0) {
